...
def get_lat_long(date):
    ...
    cs.execute("""
        ...
        """, [date])
    result = [models.LatLong(timestamp, latitude, longitude) for timestamp, latitude, longitude in cs.fetchall()]
    cs.close()
    return result
...
def get_covid19(date):
    ...
    cs.execute("""
        ...
        WHERE DATE(timestamp) =%s
        """, [date])
    result = [models.Covid19(timestamp, newcovid, totalcovid) for timestamp, newcovid, totalcovid in cs.fetchall()]
    cs.close()
    return result    
...
def get_provinces_population():
    ...
    cs.execute("""
        ...
        FROM population
        """)
    result = [models.ProvincePopulation(province, population) for province, population in cs.fetchall()]
    cs.close()
    return result    
...
def get_covid19_trend():
    ...
    cs.execute("""
        ...
        """)
    result = [models.CovidTrend(province, population, newcovid) for province, population, newcovid in cs.fetchall()]
    cs.close()
    return result 